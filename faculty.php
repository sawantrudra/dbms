...
<html>
  ...
  <input type="text" name="search" placeholder="Search..">
    ...
    <th>'.'Name'.'</th><th>'.'Office No.'.'</th><th>'.'Mobile no.'.'</th><th>'.'Email'.'</th><th>'.'Gender'.'</th><th>'.'Birthdate'.'</th></thead>';
	while(($row = $result->fetch_assoc()) && $ctr > 0)
	{
        $ctr--;
       
		echo '<tr><td>'.$row['Faculty_name'].'</td>';
        echo '<td>'.$row['Faculty_office_phone'].'</td>';
        echo '<td>'.$row['Faculty_mobile_no'].'</td>';
       
        echo '<td>'.$row['Faculty_email'].'</td>';
        echo '<td>'.$row['Gender'].'</td>';
         echo '<td>'.$row['Birthdate'].'</td>';
          echo '<td><a href="faculty_view_details.php?fid='.$row['Faculty_id'].'"class="btn btn-default btn-flat">View all Details</a></td>';  
	}
                      echo '</table>';
?>
